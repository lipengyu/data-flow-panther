...
/**
  ...
  * @Version 1.0
  * @Description 埋点数据处理主类
  */
object FlinkDealWithMallLog {
...
*/
    ...
    val kafkaTopic = PropertiesUtils.loadBuriedPointProperties("buriedPointTopic")
    ...
    //配置kerberos的三个重要属性
    kafkaProps.setProperty("group.id", PropertiesUtils.loadBuriedPointProperties("buriedPointConsumerGroup"))
    //超时时间
    ...
    //HBase属性
    val tableName = PropertiesUtils.loadBuriedPointProperties("hbase_tableName")
    ...
    val FilterData = transac.map(line => line.replaceAll("\\$", "")).
      //&& line.contains("time")
      filter(line => line.contains("uuid") && isJSONObject(line))
    ...
    */
    ...
    // 将数据Sink到HDFS
    val hdfsPath = PropertiesUtils.loadBuriedPointProperties("hdfsPathBuriedPoint")
    ...
    // 将数据Sink到HBase
    val HBaseSink = new HbaseSinkFunction(business, "properties_uuid", tableName, columnFamily, backupFamily, zookeeperConn, regionNum)
    correctDataJson.addSink(HBaseSink).name("Flink Sink HBase").setParallelism(sinkParallelism)

    //将数据Sink到ES,读取ES索引名
    val es_indexName = PropertiesUtils.loadBuriedPointProperties("es_indexName")
  ...
  def check_json(correctDataJson: DataStream[String]) = {
    ...
    correctDataJson.countWindowAll(1000).reduce((x, y) => x)
      .map(m => {
        ("uuid" + dateFormat.format(new Date().getTime), m)
      })
  }
